...
public:
    struct Wrapper final : RefWrapper<ServerPipe, ServerPipeMock>
    {
        ...
        sdk::OptErrorCode start(EventHandler event_handler) override
        {
            reference().event_handler_ = event_handler;
            return reference().start(event_handler);
        }

        sdk::OptErrorCode send(const ClientId client_id, const Payloads payloads) override
        {
    ...
    MOCK_METHOD(sdk::OptErrorCode, start, (EventHandler event_handler), (override));
    MOCK_METHOD(sdk::OptErrorCode, send, (const ClientId client_id, const Payloads payloads), (override));
